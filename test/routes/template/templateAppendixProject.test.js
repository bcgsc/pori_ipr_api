...
const checkTemplateAppendix = (appendixObject) => {
  ...
  expect(appendixObject).toEqual(expect.not.objectContaining({
    ...
    projectId: expect.any(Number || null),
...
describe('/templates/{template}/appendix', () => {
  ...
  beforeAll(async () => {
    ...
    project = await db.models.project.create({name: 'project-for-template-appendix-test'});
  });
  ...
  describe('GET', () => {
    ...
    beforeEach(async () => {
      // create two templates to be used in tests
...
describe('/templates/{template}/appendix/{project}', () => {
  let template1;
  let project1;
  let project2;
  ...
  beforeAll(async () => {
    ...
    project1 = await db.models.project.create({name: 'template-appendix-test-project1'});
    project2 = await db.models.project.create({name: 'template-appendix-test-project2'});
  });
  ...
  describe('GET', () => {
    ...
    test('/ - 200 Success', async () => {
      const res = await request
        .get(`/api/templates/${template.ident}/appendix/${project.ident}`)
        .auth(username, password)
        .type('json')
        .expect(HTTP_STATUS.OK);
  ...
  describe('POST', () => {
    test('/ - 201 Created', async () => {
      const res = await request
        .post(`/api/templates/${template.ident}/appendix/${project.ident}`)
        .auth(username, password)
        .type('json')
        .send(CREATE_DATA)
        .expect(HTTP_STATUS.CREATED);
  ...
  describe('PUT', () => {
    ...
    test('/ - 200 Success', async () => {
      const res = await request
        .put(`/api/templates/${template.ident}/appendix/${project.ident}`)
        .auth(username, password)
        .type('json')
        .send(UPDATE_DATA)
        .expect(HTTP_STATUS.OK);
  ...
  describe('DELETE', () => {
    ...
    afterEach(async () => {
      return deleteTestAppendix.destroy({force: true});
      return deleteTestAppendix2.destroy({force: true});
    });

    test('/ - 204 No Content', async () => {
      await request
        .delete(`/api/templates/${template.ident}/appendix/${project.ident}`)
        .auth(username, password)
        .type('json')
        .expect(HTTP_STATUS.NO_CONTENT);
  ...
  afterAll(async () => {
    ...
    return project2.destroy({force: true});
  });
});
