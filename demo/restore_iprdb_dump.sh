...
# import dump
POSTGRES_PASSWORD=$SERVICE_PASSWORD createdb -h localhost -U $SERVICE_USER -T $TEMPLATE_NAME $DATABASE_NAME
POSTGRES_PASSWORD=$SERVICE_PASSWORD pg_restore -h localhost -n public --no-acl --no-owner -Fc "$DB_DUMP_LOCATION" -d "$DATABASE_NAME" -U $SERVICE_USER;
