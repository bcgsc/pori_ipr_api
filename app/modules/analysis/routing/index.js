    ...
    this.registerEndpoint('get', `/extended/:analysisIdent(${this.UUIDregex})`, (req, res) => {
      ...
      db.models.pog_analysis.findOne(opts)
        ...
        .then(() => {
          ...
          const biopsy_regex = '([a-z]+)([0-9]+)';
          const analysis_biopsy = analysis.analysis_biopsy.match(biopsy_regex); // splitting analysis biopsy into sample type and biopsy number
          let source = null;
          _.forEach(sources, (biopsy_source) => { // checking each source for matching biopsy
