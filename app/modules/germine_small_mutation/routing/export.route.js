...
const tokenAuth = async (req, res, next) => {
  ...
  try {
    const flashToken = await db.models.flash_token.findOne({ where: { token: req.query.flash_token }, include: [{ model: db.models.user, as: 'user' }] });
    ...
    logger.error(error);
    return res.status(500).json({ message: 'Failed to query for flash token provided' });
  }
};
