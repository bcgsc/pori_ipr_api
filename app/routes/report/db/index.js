...
/**
 * Delete a report and any content associated with it (since cascade cannot be used with paranoid mode)
 *
 * @param {Number} reportId the ID of the report to be destroyed
 * @param {Object} options options to be passed to the destroy calls (ex. force)
 * @param {boolean} options.force flag to turn off paranoid deletion
 *
 * @returns {Promise} if no errors are thrown the report was deleted ok
 */
const destroyReport = async (reportId, options = {force: false}) => {
  try {
    const promises = Object.values(db.models.analysis_report.associations).map(async (association) => {
      await association.target.destroy({...options, where: {reportId}});
    });

    promises.push(async () => {
      return db.models.analysis_report.destroy({...options, where: {id: reportId}});
    });

    await Promise.all(promises);
  } catch (err) {
    logger.error(`Failed to destory parts of the report: ${reportId}`);
    throw err;
  }
};
...
module.exports = {
  createReportGenes, createReportSection, destroyReport,
};
