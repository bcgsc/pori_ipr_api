...
const {createReportGenes, createReportSection, destroyReport} = require('./db');
...
router.route('/')
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      await db.models.reportProject.findOrCreate({where: reportProjectData, defaults: reportProjectData});
    } catch (error) {
      await destroyReport(report.id, {force: true});
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({error: {message: 'Unable to create report', cause: error}});
    }
