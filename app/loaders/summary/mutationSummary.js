...
class mutationSummaryLoader {
  ...
  migrateData(row) {
    ...
    let result = {
      comparator: {
        comparator: null,
        snv: row['nsSNV-truncating'].split(' ')[0],
        snv_truncating: row['nsSNV-truncating'].split(' ')[1].replace(/(\[|\])/g, ''),
        indels: row['indels-frameshift'].split(' ')[0],
        indels_frameshift: row['indels-frameshift'].split(' ')[1].replace(/(\[|\])/g, ''),
        sv: row['sv-expressed'].split(' ')[0],
