...
module.exports = (POG, logger) => {
  ...
  // Setup Logger
  let log = logger.loader(POG.POGID, 'Summary.TumourAnalysis');
  
  // First parse in therapeutic
  let output = fs.createReadStream(nconf.get('paths:data:POGdata') + '/' + POG.POGID + '/JReport/Genomic/JReport_CSV_ODF/patient_tumour_analysis.csv')
  ...
  let parser = parse({delimiter: ',', columns: true},
    (err, result) => {
      ...
      if(result.length > 1) return deferred.reject('More than one patient tumour analysis entry found.') && new Error('['+POG.POGID+'][Loader][Summary.TumourAnalysis] More than one patient tumour analysis entry found.');
    
      // Remap results
      let entry = _.head(remapKeys(result, nconf.get('columnMapping:summary:tumourAnalysis')));
