...
// TODO - keep all users needed for deployment - pori_admin, iprdemo, etc
const cleanUsers = async (queryInterface, transaction, reportsToKeep) => {
  ...
  console.log('create the pori_admin user if not exists');
  ...
  await addDemoUserToGroup(queryInterface, transaction, poriAdmin, 'admin');
  ...
  await queryInterface.sequelize.query(
    `UPDATE reports_therapeutic_targets set updated_by = :demouser WHERE updated_by is not null
      ...
      and (report_id IN(:reportsToKeep));
      DELETE FROM users WHERE username != :username and username != :username2;`,
    {transaction, replacements: {username: 'iprdemo', username2: 'pori_admin', demouser: demoUser.id, reportsToKeep}},
  );
