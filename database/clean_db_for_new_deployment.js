...
const cleanUsers = async (queryInterface, transaction, reportsToKeep) => {
  ...
  let [poriAdmin] = await queryInterface.sequelize.query(
    'SELECT * FROM users where username = :username AND deleted_at IS NULL',
    {
      transaction,
      type: queryInterface.sequelize.QueryTypes.SELECT,
  ...
  if (!poriAdmin) {
    [[poriAdmin]] = await queryInterface.sequelize.query(
      `INSERT INTO users (
        ident, username, password, type,
        "firstName", "lastName", "email",
        created_at, deleted_at, updated_at
  ...
  console.log(poriAdminUserMetadata);
  if (!poriAdminUserMetadata) {
    await queryInterface.sequelize.query(
